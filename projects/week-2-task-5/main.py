...
def build_context(system_prompt: str, messages: list[dict[str, str]], strategy: str = "sliding_window",
                  window_turns: int = 3, facts: dict | None = None) -> list[dict[str, str]]:
...
class ContextStore:
    ...
    def finish_request(self, owner_id: str, chat_id: str, request_id: str, messages: list[dict[str, str]], total_tokens: int, turn_metrics: list[dict], facts: dict | None = None) -> dict:
...
class ChatAgent:
    ...
    def ask(self, user_request: str) -> str:
        ...
        context = build_context(self.system_prompt, self.messages[1:], self.strategy, self.window_turns, facts)
        estimate = estimate_context(context, user_request, self.config)
        if estimate["overflow"]:
            raise ContextLimitError(estimate)
        answer, usage = self.complete([*context, user_message])
