...
def task_action(task: dict, data: dict) -> dict:
    result = dict(task)
    action = data.get("action")
    if not isinstance(action, str):
        raise ValueError("Expected a task action string")
    if action == "update":
        if set(data) != {"action", "current_step", "expected_action"}:
            raise ValueError("Expected task step and action")
        result.update({key: bounded_text(data[key], 500) for key in ("current_step", "expected_action")})
    elif set(data) != {"action"}:
        raise ValueError("Expected one task action")
    elif action in {"pause", "resume"}:
        result["paused"] = action == "pause"
    elif action in {"advance", "rework"}:
        transitions = {"planning": "execution", "execution": "validation", "validation": "done"}
        if task["paused"] or (action == "rework" and task["stage"] != "validation") or (action == "advance" and task["stage"] not in transitions):
            raise RuntimeError("Недопустимый переход. Возобновите задачу и соблюдайте порядок этапов.")
        result["stage"] = "execution" if action == "rework" else transitions[task["stage"]]
    else:
...
def build_context(system_prompt: str, messages: list[dict[str, str]], strategy: str = "sliding_window",
    ...
    if state:
        ...
        if rules and any(value not in ("", None) for value in rules.values()):
            context[0]["content"] += (
                " Return ONLY a JSON object with fields language (string), architecture (string), budget (integer), answer (string)."
...
class ContextStore:
    ...
    def dto(self, row: sqlite3.Row, detail: bool = True) -> dict:
        ...
        if detail:
            ...
            result.update(day=DAY, memory={"short_term": validate_facts(json.loads(row["short_term"])),
                          ...
                          task=(json.loads(row["task"]) if row["task"] else initial_task()) if DAY >= 13 else None,
    ...
    def update_state(self, owner_id: str, chat_id: str, field: str, value) -> dict:
        if field not in {"short_term", "working", "profile_id", "task", "invariants"}:
...
class ChatAgent:
    ...
    def ask(self, user_request: str) -> str:
        ...
        if check and check["status"] == "blocked":
            ...
            answer, usage = self.complete([*context, user_message], json_mode=True) if constrained else self.complete([*context, user_message])
            if constrained:
                try:
                    raw = json.loads(answer)
                    if not isinstance(raw, dict) or set(raw) != {"language", "architecture", "budget", "answer"}:
                    ...
                    raise RuntimeError("Model returned invalid structured solution") from error
