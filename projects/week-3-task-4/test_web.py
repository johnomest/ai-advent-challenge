...
class WebTests(unittest.TestCase):
    ...
    def test_task_transitions_pause_restart_and_invalid_actions(self, provider):
        cookie = self.new_owner()
        path = self.new_chat(cookie)
        update = {"action": "update", "current_step": "Review plan", "expected_action": "Approve"}
        self.assertEqual(self.request("PATCH", path + "/task", update, cookie)[0], 200)
        for stage in ("planning", "execution", "validation", "done"):
            self.assertEqual(self.request(path=path, cookie=cookie)[1]["chat"]["task"]["stage"], stage)
            self.assertEqual(self.request("PATCH", path + "/task", {"action": "pause"}, cookie)[0], 200)
            self.assertEqual(self.request("POST", path + "/messages", {"message": "Continue", "request_id": "pause_" + stage}, cookie)[0], 409)
            self.assertEqual(self.request("PATCH", path + "/task", {"action": "advance"}, cookie)[0], 409)
            self.stop_server()
            self.start_server()
            ...
            self.assertEqual(self.request("PATCH", path + "/task", {"action": "resume"}, cookie)[0], 200)
            if stage == "validation":
                self.assertEqual(self.request("PATCH", path + "/task", {"action": "rework"}, cookie)[1]["chat"]["task"]["stage"], "execution")
                self.request("PATCH", path + "/task", {"action": "advance"}, cookie)
            self.assertEqual(self.request("PATCH", path + "/task", {"action": "advance"}, cookie)[0], 409 if stage == "done" else 200)
        for invalid in ({"stage": "done"}, {"action": []}, {"action": "update", "current_step": True, "expected_action": "x"}):
    ...
    def test_invariants_validate_actual_model_answer_and_atomic_failure(self, provider):
        ...
        solution = {"language": "Python", "architecture": "monolith", "budget": 500, "answer": "Use a small Python application."}
