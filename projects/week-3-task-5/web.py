...
from main import (AgentConfig, ChatAgent, ContextStore, ContextLimitError, DATA_DIR,
                  DAY, build_context, decode_metrics, estimate_context, validate_strategy,
                  validate_rules, TaskConflict, unique_object)
...
ASSETS = {
    ...
    "/week.js": ("week.js", "text/javascript; charset=utf-8"),
...
CHAT_ROUTE = re.compile(r"/api/chats/([A-Za-z0-9_-]{1,128})(/messages|/token-preview|/branches|/memory|/task|/invariants)?")
PROFILE_ROUTE = re.compile(r"/api/profiles/([A-Za-z0-9_-]{1,128})")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def preview(self, chat: dict, message: str, proposal: dict | None = None) -> dict:
        context = build_context(ChatAgent.system_prompt, chat["messages"], chat.get("strategy", "sliding_window"),
                                chat.get("window_turns", 3), chat.get("facts", {}), {**chat, "proposal": proposal})
        ...
        estimate.update(strategy=chat.get("strategy", "sliding_window"), total_turns=turns,
                        ...
                        memory_counts={key: len(value) for key, value in chat.get("memory", {}).items()},
                        profile=chat.get("profile"), task=chat.get("task"), invariants=chat.get("invariants"))
        return estimate
    ...
    def send_message(self, owner_id: str, chat: dict, data: dict) -> dict:
        if (
            set(data) not in ({"message", "request_id"}, {"message", "request_id", "proposal"})
        ...
        if "proposal" in data:
            if DAY < 14:
                raise RequestError(400, "invalid_input", "Проверка решений доступна с дня 14.")
            try:
                proposal = validate_rules(data["proposal"], proposal=True)
            except ValueError as error:
                raise RequestError(400, "invalid_input", str(error)) from error
        fingerprint = json.dumps({"message": message, "proposal": proposal}, ensure_ascii=False, sort_keys=True) if proposal else message
        prior = self.server.store.get_request(owner_id, request_id)
        if prior:
            return self.replay(prior, chat["id"], fingerprint)
        if chat.get("task") and chat["task"]["paused"]:
            raise RequestError(409, "task_paused", "Задача на паузе. Нажмите «Продолжить».")
        if chat["task"]["stage"] == "done":
            raise RequestError(409, "task_done", "Задача завершена. Создайте новую задачу.")
        estimate = self.preview(chat, message, proposal)
        ...
        prior = self.server.store.reserve_request(owner_id, chat["id"], request_id, fingerprint)
        if prior:
            return self.replay(prior, chat["id"], fingerprint)
        try:
            ...
            agent.state = {**chat, "proposal": proposal}
            agent.ask(message)
        ...
        return self.server.store.finish_request(owner_id, chat["id"], request_id, agent.messages[1:], agent.total_tokens, agent.turn_metrics, agent.facts, agent.last_check)

    def extended_request(self, owner_id: str, match, data: dict) -> dict | None:
        ...
        try:
            if self.path == "/api/memory":
                if self.command == "PATCH":
                    ...
                    return {"memory": store.set_memory(owner_id, data["entries"])}
                return {"memory": store.get_memory(owner_id)}
            profile_match = PROFILE_ROUTE.fullmatch(self.path)
            if self.path == "/api/profiles" or profile_match:
                if DAY < 12:
                    raise KeyError("profiles")
                profile_id = profile_match[1] if profile_match else None
                if profile_id and store.get_profile(owner_id, profile_id) is None:
                    raise KeyError(profile_id)
                if self.command in {"POST", "PATCH"}:
                    profile = store.save_profile(owner_id, data, profile_id)
                    return {"profile": profile, "profiles": store.list_profiles(owner_id)}
                if self.command == "DELETE":
                    if data:
                        raise ValueError("Expected empty body")
                    store.delete_profile(owner_id, profile_id)
                return {"profiles": store.list_profiles(owner_id)}
            if not match or match[2] not in {"/memory", "/task", "/invariants"}:
                return None
            chat = store.get_chat(owner_id, match[1])
            resource = match[2][1:]
            if chat is None or (resource == "task" and DAY < 13) or (resource == "invariants" and DAY < 14):
                raise KeyError(resource)
            if self.command == "PATCH":
                if resource == "memory":
                    if set(data) != {"layer", "entries"} or not isinstance(data["layer"], str) or data["layer"] not in {"short_term", "working"}:
                        raise ValueError("Choose short_term or working and provide entries")
                    chat = store.update_state(owner_id, chat["id"], data["layer"], data["entries"])
                elif resource == "task":
                    try:
                        chat = store.apply_task_action(owner_id, chat["id"], data)
                    except TaskConflict as error:
                        raise RequestError(409, error.code, str(error)) from error
                else:
                    chat = store.update_state(owner_id, chat["id"], "invariants", data)
            return {"chat": self.chat_dto(chat, self.server.is_busy(owner_id) if self.command == "GET" else False)}
        except ValueError as error:
            raise RequestError(400, "invalid_input", str(error)) from error
        except KeyError as error:
            raise RequestError(404, "not_found", "Ресурс не найден.") from error
    ...
    def handle_request(self) -> None:
        ...
        try:
            ...
            extra = ((self.path == "/api/memory" and self.command in {"GET", "PATCH"})
                     or (self.path == "/api/profiles" and self.command in {"GET", "POST"})
                     or (PROFILE_ROUTE.fullmatch(self.path) and self.command in {"PATCH", "DELETE"})
                     or (match and match[2] in {"/memory", "/task", "/invariants"} and self.command in {"GET", "PATCH"}))
            ...
            detail = match and ((match[2] is None and self.command in {"GET", "PATCH", "DELETE"})
                                or (match[2] in {"/messages", "/token-preview", "/branches"} and self.command == "POST"))
            if not collection and not detail and not extra:
            ...
            status = 200
            if extra:
                result = self.extended_request(owner_id, match, data)
                self.json_reply(201 if self.command == "POST" else 200, result, owner_id)
                return
            if collection:
                if self.command == "GET":
                ...
                if preview_route:
                    if set(data) not in ({"message"}, {"message", "proposal"}) or not isinstance(data["message"], str) or len(data["message"]) > 4000:
                        raise RequestError(400, "invalid_input", "Сообщение должно содержать не более 4000 символов.")
                    proposal = None
                    if "proposal" in data:
                        try:
                            if DAY < 14:
                                raise ValueError("Proposal is only available on day 14")
                            proposal = validate_rules(data["proposal"], proposal=True)
                        except ValueError as error:
                            raise RequestError(400, "invalid_input", str(error)) from error
                    self.json_reply(200, {"estimate": self.preview(chat, data["message"], proposal)}, owner_id)
                    return
                if match[2] == "/branches":
                ...
                if self.command == "DELETE":
                    ...
                    if self.command == "PATCH":
                        if set(data) == {"profile_id"}:
                            if DAY < 12 or (data["profile_id"] is not None and not isinstance(data["profile_id"], str)):
                                raise RequestError(400, "invalid_input", "Выберите профиль.")
                            try:
                                self.server.store.update_state(owner_id, chat_id, "profile_id", data["profile_id"])
                            except KeyError as error:
                                raise RequestError(404, "not_found", "Профиль не найден.") from error
                        elif set(data) == {"title"}:
